...
def main():
    try:
        ...
        # Check for rate limit information in Claude Code messages
        # This captures timing messages that Claude Code provides directly
        if prompt and isinstance(prompt, str):
            # Check if this might be a rate limit related message
            prompt_lower = prompt.lower()
            if any(keyword in prompt_lower for keyword in [
                'rate limit', 'rate limited', 'too many requests', 'quota exceeded',
                'calcolo tempo di reset', 'esperienza storica', 'reset basato'
            ]):
                rate_limit_data = parse_and_store_rate_limit(prompt, session_id)
                if rate_limit_data:
                    print(f"📊 Claude Code rate limit captured: {rate_limit_data['provider']} - {rate_limit_data.get('reset_seconds', 'unknown')}s", file=sys.stderr)
