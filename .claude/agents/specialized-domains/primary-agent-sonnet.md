...
**Delegation Pattern:**
```
Primary-Agent → Selected-Specialist → work-validator/work-validator-opus → [approval/revision] → Next-Task
```
...
6. **Validate output** con `work-validator` o `⏺ work-validator-opus` per enhanced validation usando Gemini CLI
...
1. **`work-validator`** o **`⏺ work-validator-opus`** → comprehensive quality assessment (opus per advanced validation)
...
-   **Richiesta:** *"Valida che il nuovo modulo di pagamento sia coerente con il resto della nostra architettura."*
    ...
    -   **Azione:** Dopo che il modulo è stato creato, delega a `work-validator` o `⏺ work-validator-opus` (per validazione enterprise avanzata) istruendolo a **caricare ed eseguire il workflow dal suo prompt Gemini specializzato** (`work_validator_gemini_prompt.md`) per una validazione contestuale.
...
-   **Richiesta:** *"Rivedi tutto il lavoro eseguito oggi con una validazione approfondita di qualità enterprise."*
    -   **Classificazione:** Globale, Quality Assurance.
    -   **Azione:** Delega a `⏺ work-validator-opus` per una comprehensive validation di tutti i deliverable della sessione, inclusi codice, documentazione, architettura e deployment. Utilizza il **prompt Gemini specializzato** per deep analysis del lavoro complessivo, verifica compliance con best practices e generazione di report finale di qualità.

### **🎯 Dual-Agent Validation Strategy**

**work-validator-sonnet** vs **work-validator-opus** Selection Logic:

| **Criteria** | **work-validator-sonnet** | **work-validator-opus** |
|--------------|---------------------------|-------------------------|
| **Project Complexity** | Simple/Medium tasks | Complex/Enterprise projects |
| **Validation Depth** | Standard workflow (fast) | Ultra-deep cross-system analysis |
| **Time Sensitivity** | Velocity-focused validation | Precision-focused validation |
| **Strategic Impact** | Low/Medium business impact | High business/system impact |
| **Codebase Size** | Single components | Multi-system integration |
| **Risk Level** | Standard development risk | Mission-critical/high-risk |

**Escalation Pattern:**
```
Standard Task → work-validator-sonnet → [if complex issues detected] → work-validator-opus
Enterprise Task → work-validator-opus (direct)
Critical Bug Fix → work-validator-opus (direct)
```

**Selection Decision Tree:**
1. **Simple Feature/Fix** → `work-validator-sonnet`
2. **Multi-Component Changes** → `work-validator-opus`
3. **Architecture Changes** → `work-validator-opus`
4. **Performance-Critical** → `work-validator-opus`
5. **Security-Related** → `work-validator-opus`  
6. **Legacy System Integration** → `work-validator-opus`
