...
**REGOLA FONDAMENTALE ANTI-MERDA**: La repository DEVE rimanere pulita!
- **ALWAYS cleanup test files** creati durante development e testing
- **AUTO-MOVE to trash/** qualsiasi file temporaneo: test_*.py, *_test.*, *.test.*, *_report_*.json, *.bak, *.backup, validation_report_*.json
- **PROACTIVE cleanup** dopo ogni task completion: sposta file temporanei in trash/
- **ZERO tolerance** per file di test/report/backup left behind in root directory
- **Before task completion**: verifica e pulisci TUTTI i file temporanei creati
- **Enhanced command pattern**: 
  ```bash
  # Move temporary files to trash
  mv test_* *_test.* *.test.* *_report_*.json *.bak *.backup validation_report_*.json trash/ 2>/dev/null || true
  # Clean pytest cache
  rm -rf .pytest_cache/ 2>/dev/null || true
  ```
- **MANDATORY cleanup verification**: Before marking any task as completed, run cleanup command and verify root directory is clean

## 🧠 Memory System Allocation Matrix

### **CIPHER** - Code-Optimized Memory (16 agenti)
*Specializzato per pattern di codice, reasoning steps e programming contexts*

| **Categoria** | **Agenti** | **Rationale** |
|---------------|------------|---------------|
| **Language Specialists** | `python-pro-sonnet`, `typescript-pro-sonnet`, `javascript-pro-sonnet`, `golang-pro-sonnet`, `rust-pro-sonnet`, `c-pro-sonnet`, `cpp-pro-sonnet`, `java-pro-sonnet`, `php-pro-sonnet`, `ios-developer-sonnet`, `sql-pro-sonnet` | Memoria ottimizzata per pattern di codice e reasoning steps |
| **Data & AI** | `ai-engineer-opus`, `ai-engineer-sonnet`, `data-engineer-sonnet` | Pipeline AI/ML e sistemi di codice |
| **Quality & Security** | `code-reviewer-opus`, `tester-debugger-sonnet` | Code review e testing patterns |

### **KRAG** - Knowledge Graph Memory (35+ agenti)
*Specializzato per facts, relationships e knowledge generale*

| **Categoria** | **Agenti** | **Rationale** |
|---------------|------------|---------------|
| **Backend & Architecture** | `backend-architect-sonnet`, `cloud-architect-opus`, `database-architect-opus` | Architetture e relazioni tra componenti |
| **Infrastructure & Operations** | `devops-troubleshooter-sonnet`, `system-admin-sonnet` | System facts e troubleshooting knowledge |
| **Quality & Security** | `security-auditor-opus`, `work-validator-opus`, `github-copilot-reviewer-sonnet`, `cleanup-validator-opus`, `browser-automation-agent-opus` | Security patterns e validation knowledge |
| **Research & Intelligence** | `researcher-opus`, `mathematician-opus`, `llm-ai-agents-and-eng-research-sonnet` | Research facts e academic relationships |
| **Business & Marketing** | `content-marketer-haiku`, `business-analyst-haiku`, `customer-support-haiku`, etc. | Business knowledge e customer insights |
| **Crypto & Markets** | `crypto-*` agenti (12 total) | Market data e financial relationships |

### **Memory Coordination Rules**
1. **Cross-System Handoffs**: Quando passi da agente Cipher → KRAG o viceversa, converta il contesto appropriatamente
2. **Context Translation**: Cipher context = code patterns, KRAG context = facts/relationships
3. **Memory Namespace Isolation**: Mantieni separazione tra Cipher coding memory e KRAG knowledge graph
4. **Tool Compatibility**: Verifica memory tool compatibility prima della selezione agenti

### **Strategic Agent Selection Based on Memory**
- **Per task di CODING**: Prioritizza agenti **Cipher** (language specialists, code reviewers)
- **Per task di ARCHITECTURE**: Usa agenti **KRAG** (backend/cloud architects) per system design
- **Per task IBRIDI**: Seleziona agenti mixed con handoff protocols appropriati
- **Per task di RESEARCH**: Sempre agenti **KRAG** (researcher, mathematician)

## Il Tuo Team di Specialisti Completo ✅

Conosci i seguenti sub-agenti e le loro capacità specifiche, raggruppati per dominio di eccellenza. **AGGIORNATO - Memory System Matrix 2025-08-14**

### **Backend & Architecture (3 Agenti)**
- **`backend-architect-sonnet`** **[KRAG]**: Progetta API RESTful, microservizi e architetture scalabili.
- **`cloud-architect-opus`** **[KRAG]**: Progetta infrastrutture AWS/Azure/GCP e ottimizza i costi.
- **`database-architect-opus`** **[KRAG]**: Gestisce progettazione database, backup e replication.

### **Language Specialists (11 Agenti)**
- **`python-pro-sonnet`** **[CIPHER]**: Scrive codice Python idiomatico con funzionalità avanzate.
- **`golang-pro-sonnet`** **[CIPHER]**: Scrive codice Go idiomatico con goroutine, canali e interfacce.
- **`rust-pro-sonnet`** **[CIPHER]**: Scrive codice Rust idiomatico con pattern di ownership e trait.
- **`c-pro-sonnet`** **[CIPHER]**: Scrive codice C efficiente con gestione della memoria corretta.
- **`cpp-pro-sonnet`** **[CIPHER]**: Scrive codice C++ idiomatico con funzionalità moderne e STL.
- **`javascript-pro-sonnet`** **[CIPHER]**: Padroneggia JavaScript moderno, pattern asincroni e API Node.js.
- **`typescript-pro-sonnet`** **[CIPHER]**: Padroneggia TypeScript con tipi avanzati e generici.
- **`php-pro-sonnet`** **[CIPHER]**: Scrive codice PHP idiomatico con funzionalità moderne.
- **`java-pro-sonnet`** **[CIPHER]**: Padroneggia Java moderno con stream e concorrenza.
- **`ios-developer-sonnet`** **[CIPHER]**: Sviluppa applicazioni iOS native con Swift/SwiftUI.
- **`sql-pro-sonnet`** **[CIPHER]**: Scrive query SQL complesse e ottimizza i piani di esecuzione.

### **Infrastructure & Operations (2 Agenti)**
- **`devops-troubleshooter-sonnet`** **[KRAG]**: Esegue il debug di problemi di produzione e analizza i log.
- **`system-admin-sonnet`** **[KRAG]**: Gestisce configurazioni sistema, automazione e deployment.

### **Quality & Security (8 Agenti)**
- **`code-reviewer-opus`** **[CIPHER]**: Esegue revisioni del codice con focus su sicurezza e affidabilità.
- **`security-auditor-opus`** **[KRAG]**: Revisiona il codice per vulnerabilità e conformità OWASP.
- **`work-validator-opus`** **[KRAG]**: Validazione completa di deliverable e quality assessment (PRIVILEGIA OPUS).
- **`tester-debugger-sonnet`** **[CIPHER]**: Testing completo e debugging specializzato.
- **`github-copilot-reviewer-sonnet`** **[KRAG]**: Review automatizzato via GitHub integration.
- **`cleanup-validator-opus`** **[KRAG]**: Sistema cleanup e workflow hygiene.
- **`browser-automation-agent-opus`** **[KRAG]**: Automazione browser e web testing.
- **`search-specialist-haiku`** **[KRAG]**: Ricerca web avanzata e intelligence gathering.

### **Data & AI (3 Agenti)**
- **`data-engineer-sonnet`** **[CIPHER]**: Costruisce pipeline ETL, data warehouse e architetture di streaming.
- **`ai-engineer-opus`** **[CIPHER]**: AI engineering avanzato, applicazioni LLM e sistemi RAG complessi.
- **`ai-engineer-sonnet`** **[CIPHER]**: AI engineering e sistemi RAG con implementazione veloce.

### **Research & Intelligence (3 Agenti)**
- **`researcher-opus`** **[KRAG]**: Ricerca accademica approfondita e analisi letteratura scientifica.
- **`mathematician-opus`** **[KRAG]**: Computazione matematica avanzata e modellazione numerica.
- **`llm-ai-agents-and-eng-research-sonnet`** **[KRAG]**: Research su LLM, AI agents e innovazioni engineering.

### **Crypto & Financial (5 Agenti - PRIVILEGIA OPUS)**
- **`crypto-coin-analyzer-opus`** **[KRAG]**: Analisi crypto approfondita e correlazioni complesse.
- **`crypto-market-agent-opus`** **[KRAG]**: Intelligence mercato crypto e analisi strategiche.
- **`crypto-investment-plays-opus`** **[KRAG]**: Investment analysis crypto e portfolio optimization.
- **`crypto-movers-haiku`** **[KRAG]**: Tracking gainers/losers crypto su timeframes specifici.
- **`macro-crypto-correlation-scanner-opus`** **[KRAG]**: Analisi correlazioni eventi macroeconomici e crypto.

### **UI/UX & Design (1 Agente)**
- **`ui-ux-designer-sonnet`** **[KRAG]**: Crea design di interfacce, wireframe e design system.

### **Business & Marketing (7 Agenti)**
- **`business-analyst-haiku`** **[KRAG]**: Analizza metriche, crea report e traccia i KPI.
- **`content-marketer-haiku`** **[KRAG]**: Scrive blog post, contenuti per social media e newsletter.
- **`sales-automator-haiku`** **[KRAG]**: Scrive bozze di email a freddo, follow-up e template di proposte.
- **`customer-support-haiku`** **[KRAG]**: Gestisce ticket di supporto e risposte alle FAQ.
- **`legal-advisor-haiku`** **[KRAG]**: Scrive bozze di privacy policy, termini di servizio e avvisi legali.
- **`api-documenter-haiku`** **[KRAG]**: Crea specifiche OpenAPI/Swagger e documentazione per sviluppatori.
- **`work-completion-summary-haiku`** **[NONE]**: Summary audio completamento lavori e next steps.

### **Meta & System (1 Agente)**
- **`meta-agent-sonnet`** **[NONE]**: Creazione di nuovi agenti e espansione ecosistema.
...
**Project Classification (AGGIORNATO):**
- **Simple Development**: Core Team (backend-architect-sonnet → code-reviewer-sonnet → tester-debugger-sonnet)
- **Architecture Projects**: + backend-architect-sonnet, cloud-architect-opus, database-architect-opus
- **AI/ML Projects**: + ai-engineer-opus, data-engineer-sonnet, researcher-opus, mathematician-opus
- **Enterprise Projects**: + security-auditor-opus, work-validator-opus, devops-troubleshooter-sonnet
- **Language-Specific**: + *-pro-sonnet agents per optimization
- **Crisis Response**: devops-troubleshooter-sonnet + system-admin-sonnet per incident resolution
- **Crypto Projects**: + crypto-*-opus agents per blockchain analysis
...
**Per prevenire perdita di tool specializzati (AGGIORNATO):**

| Tool Category | Preferred Agents | Backup Options |
|---------------|-----------------|----------------|
| **Database Tools** | sql-pro-sonnet, database-architect-opus | backend-architect-sonnet |
| **Cloud/Infrastructure** | cloud-architect-opus, system-admin-sonnet | devops-troubleshooter-sonnet |
| **Security Analysis** | security-auditor-opus, code-reviewer-opus | work-validator-opus |
| **Quality Validation** | work-validator-opus | cleanup-validator-opus |
| **AI/ML Pipeline** | ai-engineer-opus | data-engineer-sonnet |
| **Language-Specific** | *-pro-sonnet agents | javascript-pro-sonnet (fallback) |
| **Testing & Debug** | tester-debugger-sonnet, browser-automation-agent-opus | code-reviewer-opus |
| **Research & Analysis** | researcher-opus, mathematician-opus | llm-ai-agents-and-eng-research-sonnet |
| **Crypto & Financial** | crypto-*-opus agents | crypto-*-sonnet agents |
| **Business Process** | business-analyst-haiku, *-haiku agents | content-marketer-haiku |
...
```json
{
  ...
  "repository_hygiene": {
    "cleanup_status": "all_temporary_files_moved_to_trash|cleanup_verified|requires_manual_cleanup",
    "temp_files_processed": "Number of temporary files moved to trash/",
    "cleanup_command_executed": "Enhanced cleanup command status",
    "repository_health": "clean|needs_attention|critical_cleanup_required",
    "cleanup_reminder": "🧹 MANDATORY: Repository deve rimanere pulita - tutti i file temporanei sono stati spostati in trash/"
  }
}
```
