import { initDatabase, insertEvent, getFilterOptions, getRecentEvents, getAgentStats, getAgentMetadata, getAgentActivity } from './db';
...
const server = Bun.serve({
  ...
  async fetch(req: Request) {
    ...
    // POST /api/events - Receive new events (with agent metadata support)
      ...
      try {
        ...
        const health = {
          ...
          multi_agent: {
            ...
            agent_categories: Object.keys(stats.agents_by_category)
          },
          database: 'connected',
    ...
    // GET /api/agents/metadata - Get all agent metadata
    if (url.pathname === '/api/agents/metadata' && req.method === 'GET') {
      try {
        ...
        return new Response(JSON.stringify({ error: 'Failed to get agent metadata' }), {
          status: 500,
          headers: { ...headers, 'Content-Type': 'application/json' }
        });
      }
    ...
    // GET /api/agents/{agentName}/activity - Get activity for specific agent
    if (url.pathname.startsWith('/api/agents/') && url.pathname.endsWith('/activity') && req.method === 'GET') {
      try {
        const pathParts = url.pathname.split('/');
        const agentName = pathParts[3]; // Extract agent name from path
        const limit = parseInt(url.searchParams.get('limit') || '50');
        
        const activity = getAgentActivity(agentName, limit);
        return new Response(JSON.stringify(activity), {
          headers: { ...headers, 'Content-Type': 'application/json' }
        });
      } catch (error) {
        console.error('Error getting agent activity:', error);
        return new Response(JSON.stringify({ error: 'Failed to get agent activity' }), {
          status: 500,
          headers: { ...headers, 'Content-Type': 'application/json' }
        });
      }
